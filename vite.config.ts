...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    define: {
      ...
      'process.env.FIREBASE_MEASUREMENT_ID': JSON.stringify(env.VITE_FIREBASE_MEASUREMENT_ID || env.FIREBASE_MEASUREMENT_ID),
      'process.env.FIREBASE_GOOGLE_CLIENT_ID': JSON.stringify(env.VITE_FIREBASE_GOOGLE_CLIENT_ID || env.FIREBASE_GOOGLE_CLIENT_ID),
    },
    server: {
      port: 3000,
      host: '0.0.0.0',
      // Proxy removed as it is no longer required for Firebase communication
    },
  }
})
